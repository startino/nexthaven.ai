...
class BookingApifyRequest(BaseModel):
    ...
    currency: str = Field(
        default="USD", description="The currency to use for the search"
    )
    language: str = Field(
        default="en-gb", description="The language to use for the search"
    )
    rooms: int = Field(default=1, description="The number of rooms to search for")
    ...
    minMaxPrice: str = Field(
        default="0-2000", description="The minimum and maximum price to search for"
    )
