...
class AirbnbApifyAgent:
    ...
    async def get_properties(self, request: AirbnbApifyRequest) -> list[AirbnbApifyResponse]:
        """
        This method is used to get properties from the Apify API for Airbnb.
        """
        run_input = request.model_dump(exclude_none=True)
        
        # Add maxItems parameter to limit the number of results
        ...
        run = await loop.run_in_executor(
            None,
            lambda: self.client.actor(self.actor_id).call(run_input=run_input, max_items=max_items)
        )
    ...
    def _parse_airbnb_item(self, item: dict) -> AirbnbApifyResponse:
        ...
        try:
            ...
            if "houseRules" in item and item["houseRules"]:
                try:
                    general_rules = []
                    if "general" in item["houseRules"] and isinstance(item["houseRules"]["general"], list):
                        for rule_group_data in item["houseRules"]["general"]:
                            values = []
                            if "values" in rule_group_data and isinstance(rule_group_data["values"], list):
                            ...
                            general_rules.append(RuleGroup(
            ...
            if "amenities" in item and isinstance(item["amenities"], list):
                for amenity_group_data in item["amenities"]:
                    values = []
                    if "values" in amenity_group_data and isinstance(amenity_group_data["values"], list):
                        for value_data in amenity_group_data["values"]:
                            values.append(AmenityValue(
                                title=value_data.get("title", ""),
                                subtitle=value_data.get("subtitle", ""),
                                icon=value_data.get("icon"),
                    ...
                    amenities.append(AmenityGroup(
                        title=amenity_group_data.get("title", ""),
                        values=values
                    ))
            
            # Extract location descriptions
            location_descriptions = []
            if "locationDescriptions" in item and isinstance(item["locationDescriptions"], list):
                for loc_desc_data in item["locationDescriptions"]:
                    location_descriptions.append(LocationDescription(
                        title=loc_desc_data.get("title", ""),
                        content=loc_desc_data.get("content"),
                        mapMarkerRadiusInMeters=loc_desc_data.get("mapMarkerRadiusInMeters")
                    ))
            ...
            if "price" in item and item["price"]:
                ...
                if "breakDown" in price_data and price_data["breakDown"]:
                    ...
                    if "basePriceBreakdown" in bd_data and isinstance(bd_data["basePriceBreakdown"], list):
                        for bp_item in bd_data["basePriceBreakdown"]:
                            base_price_breakdown.append(PriceBreakdownItem(
                    ...
                    if "totalBeforeTaxes" in bd_data and bd_data["totalBeforeTaxes"]:
                        total_before_taxes = PriceBreakdownItem(
                            description=bd_data["totalBeforeTaxes"].get("description", ""),
                    ...
                    if "earlyBirdDiscount" in bd_data and bd_data["earlyBirdDiscount"]:
                        early_bird_discount = PriceBreakdownItem(
                            description=bd_data["earlyBirdDiscount"].get("description", ""),
            ...
            if "htmlDescription" in item and item["htmlDescription"]:
                html_description = HtmlDescription(
                    ...
                    recommendedNumberOfLines=item["htmlDescription"].get("recommendedNumberOfLines")
                )
            ...
            if "cancellationPolicies" in item and isinstance(item["cancellationPolicies"], list):
                for policy_data in item["cancellationPolicies"]:
                    cancellation_policies.append(CancellationPolicy(
...
if __name__ == "__main__":
    ...
    location = "New York"
    check_in = (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d")  # 30 days from now
    check_out = (datetime.now() + timedelta(days=37)).strftime("%Y-%m-%d")  # 7 days stay
    ...
    if properties:
        ...
        for i, prop in enumerate(properties):
            ...
            print(f"   Rating: {prop.rating.guestSatisfaction if prop.rating else 'N/A'} ({prop.rating.reviewsCount if prop.rating and prop.rating.reviewsCount else 0} reviews)")
            print(f"   Host: {prop.host.name} (SuperHost: {prop.host.isSuperHost})")
            print(f"   Capacity: {prop.personCapacity} guests")
            print(f"   Location: {prop.coordinates.latitude}, {prop.coordinates.longitude}")
