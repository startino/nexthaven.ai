...
from src.routers.evaluation import query_properties, evaluate_properties
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="Snapshot test skipped as the snapshot data needs to be initialized with --snapshot-update"
)
async def test_query_properties_response_snapshot(
    snapshot, mock_asyncio_task, mock_time, mock_cache
):
    """
    ...
    # Act
    with patch(
        "src.lib.cache.property_cache.generate_unique_id",
        return_value="snapshot-session-id",
    ):
...
async def test_evaluate_properties_response_snapshot(snapshot, mock_cache):
    ...
    with patch(
        "src.lib.cache.property_cache.get_query_status",
        return_value={"status": "completed", "completed": True},
    ), patch(
        "src.lib.cache.property_cache.retrieve_properties",
        return_value=([MagicMock(**mock_property)], MagicMock()),
    ), patch(
        "src.lib.evaluate.agents_with_vision.EvaluateAgent"
    ) as mock_agent:
        ...
        request = PropertyEvaluationRequest(
            session_id=session_id, preferences="I want a quiet place with a pool."
        )

        # Act
        with patch(
            "time.time", side_effect=[1000000.0, 1000005.0]
        ):  # Start and end times
