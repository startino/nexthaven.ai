...
from src.routers.evaluation import (
    query_properties,
...
async def test_get_query_status(mock_cache):
    ...
    with patch(
        "src.routers.property_evaluation.get_query_status", new_callable=AsyncMock
    ) as mock_get_status:
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="This test is skipped due to difficulties with mocking the complex property model. Need to revisit."
)
async def test_evaluate_properties_completed_query(
    mock_cache, mock_evaluate_agent, mock_property_data, mock_requirement_data
):
    """
    ...
    with patch(
        ...
        "src.routers.property_evaluation.retrieve_properties", new_callable=AsyncMock
    ) as mock_retrieve, patch(
        "src.routers.property_evaluation.EvaluateAgent",
        return_value=mock_evaluate_agent,
    ), patch(
        "src.routers.property_evaluation.AnalyzeUserRequirement"
    ) as mock_analyzer:
...
@pytest.mark.skip(
    reason="This test is skipped due to the create_app() function not accepting a 'testing' parameter"
)
def test_property_query_flow(
    client, mock_cache, mock_booking_apify, mock_airbnb_apify, mock_evaluate_agent
):
    """
    ...
    client.post(
        f"/properties/query/{session_id}/status",
        json={"status": "completed", "completed": True, "properties_count": 2},
    )
...
async def test_query_timeout_behavior(mock_cache):
    ...
    request = PropertyEvaluationRequest(
        session_id=session_id, preferences="I want a pool"
    )
    ...
    with patch(
        ...
        "src.routers.property_evaluation.time.time", return_value=current_time + 200
    ):
        ...
        assert "408: Property query timed out" in str(
            exc_info.value.detail
        )  # But it contains the inner 408 exception
