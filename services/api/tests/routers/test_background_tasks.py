...
from src.routers.property_evaluation import fetch_properties_background

# ========== Background Task Tests ==========

@pytest.mark.unit
@pytest.mark.asyncio
@pytest.mark.skip(reason="This test is skipped due to difficulties with mocking the async functions correctly. Need to revisit.")
async def test_fetch_properties_background_success():
    ...
    store_status_mock.side_effect = lambda sid, status: status_updates.update({sid: status})
    ...
    # Apply all patches
    with patch("src.lib.cache.property_cache.store_query_status", store_status_mock), \
         patch("src.lib.cache.property_cache.store_properties", store_properties_mock), \
         patch("src.lib.evaluate.analyze.AnalyzeUserRequirement", return_value=analyzer_mock), \
         patch("src.lib.scraper.booking_apify.BookingApifyAgent", return_value=booking_agent_mock), \
         patch("src.lib.scraper.airbnb_apify.AirbnbApifyAgent", return_value=airbnb_agent_mock):
    ...
    assert final_status["properties_count"] == len(booking_properties + airbnb_properties)
...
async def test_fetch_properties_background_error(mock_cache):
    ...
    with patch("src.lib.cache.property_cache.store_query_status", mock_store_status), \
         patch("src.lib.scraper.booking_apify.BookingApifyAgent", return_value=booking_mock):
...
async def test_concurrent_background_tasks(mock_cache):
    ...
    with patch("src.lib.cache.property_cache.store_query_status", mock_store_status), \
        ...
        tasks = [fetch_properties_background(sid, req) for sid, req in zip(sessions, requests)]
