<script lang="ts">
	...
	// 				console.log(
	// 					`Properties evaluated successfully, found: ${properties.length} at ${new Date().toISOString()}`
	// 				);
	...
	// 				redirectToSearchWithError(
	// 					'No properties found matching your criteria. Please try with different preferences.'
	// 				);
	// 			}
					...
					for (let i = 1; i < stepTimeDistribution.length; i++) {
						...
						if (timeElapsed >= prevThreshold && timeElapsed < nextThreshold) {
							...
							for (let j = 0; j < stepProgressRanges.length; j++) {
								if (
									targetProgress >= stepProgressRanges[j].min &&
									targetProgress < stepProgressRanges[j].max
								) {
									currentStep = j;
									break;
								}
...
<div class="bg-background flex min-h-screen items-center justify-center">
	<div class="w-full max-w-md space-y-8 px-4 py-6">
		<div class="space-y-4 text-center">
			<h1 class="text-gradient font-serif text-4xl">nexthaven.ai</h1>
			<p class="text-foreground/70 text-lg">Our AI agents are working their magic</p>
			<p class="text-foreground/50 text-sm">This may take a few minutes</p>

			{#if error}
				<div class="bg-destructive/20 mb-6 mt-6 w-full rounded-md p-4">
					<div class="flex items-start gap-3">
						<div class="mt-1">⚠️</div>
						<div class="flex-1">
							<h3 class="text-foreground mb-1 font-medium">We hit a snag</h3>
							<p class="text-foreground/80 text-sm">{error}</p>
			...
			{:else}
				<!-- Cancel button -->
				<Button variant="ghost" size="sm" class="mt-2 flex items-center gap-1.5" href="/search">
					<X class="h-3.5 w-3.5" />
		...
		<div class="space-y-4">
			{#each steps as step, index}
				...
				>
					<div
						class={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full transition-all duration-300
						${
							index === currentStep
								? 'gradient-primary shadow-primary/20 bg-gradient-to-r shadow-lg'
								: index < currentStep
									? 'bg-primary/50'
									: 'bg-foreground/10'
						}`}
		...
		<div class="mt-6 w-full space-y-2">
			<div class="text-foreground/70 flex w-full justify-between text-sm">
				<span>Progress: {Math.round(progress)}%</span>
				<span>Est. time remaining: {formatTime(timeRemaining)}</span>
			</div>
			<div class="bg-foreground/10 h-3 w-full overflow-hidden rounded-full">
				<div
					class="gradient-primary relative h-full rounded-full bg-gradient-to-r transition-all duration-300"
					style="width: {Math.min(progress, 100)}%; box-shadow: 0 0 10px hsl(var(--ring)/50%);"
				>
					<!-- Animated pulse effect to show activity -->
					<div class="bg-foreground/20 animate-pulse-subtle absolute inset-0"></div>
				</div>
			</div>
		</div>
	</div>
